...
def enhancedFeatureExtractorDigit(datum):
  ...
  for x in range(1, DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            ...
            #from blank to grey/black: left to right
            features[("verti", x, y)] = int(datum.getPixel(x, y) >
                                            datum.getPixel(x, y - 1))
            #from blank to grey/black: top to bottom
  ...
  #to store the point that has been visited
  visited = set()
  #nums of white space
  numofwhite = 0
  for x in xrange(DIGIT_DATUM_WIDTH):
    for y in xrange(DIGIT_DATUM_HEIGHT):
      if (x, y) not in visited and datum.getPixel(x, y) < 2:
        numofwhite += 1
        stack = [(x, y)]
        while stack:
          point = stack.pop()
          visited.add(point)
          for neighbors in getNeighbors(*point):
            if datum.getPixel(*neighbors) < 2 and neighbors not in visited:
              stack.append(neighbors)

  features["numofwhite"] = numofwhite
  return features
...
def enhancedFeatureExtractorFace(datum):
  ...
  for x in range(FACE_DATUM_WIDTH/2):
    for y in range(FACE_DATUM_WIDTH/2):
      if features[(x,y)] > 0:
        features[(x,y)] = 1
      else:
        features[(x,y)] = 0
...
def readCommand( argv ):
  ...
  parser.add_option('-t', '--training', help=default('The size of the training set'), default=5000/10*10, type="int")
